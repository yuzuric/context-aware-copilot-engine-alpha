# Context Aware Copilot Engine Alpha
...
`context-aware-copilot-engine-alpha` is built on top of [Xiaomi MiMo](https://platform.xiaomimimo.com), the open-source large language model series from Xiaomi. The project demonstrates how to integrate MiMo into production systems using its OpenAI-compatible API.
...
```python
import asyncio
from context_aware_copilot_engine_alpha.client import MiMoClient
...
├── config.yaml
├── main.py
├── src/context_aware_copilot_engine_alpha/
│   ├── reviewer.py
│   ├── client.py
│   └── config.py
├── tests/
